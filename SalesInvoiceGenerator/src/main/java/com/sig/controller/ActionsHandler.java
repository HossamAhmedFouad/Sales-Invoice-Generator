...
public class ActionsHandler {
    ...
    public ActionsHandler(FileOperations fileOperations, GUI gui) {
        ...
        //Read from resources package and initialize table with invoices data
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            //Clicking On Delete Invoice Button - "delete" command
                ...
                if (rowSelected) {
                    ...
                    gui.setInvoicesTableModel(myModel);
                    fileOperations.writeFile(invoiceHeaderArrayList);
                }
            ...
            //Clicking On Save Button - "save" command
                ...
                for (int i =0;i<invoiceHeaderArrayList.get(gui.getInvoicesHeadersTable().getSelectedRow()).getInvoiceLines().size();i++){
                    if (myModel.getValueAt(i,1).equals("")||myModel.getValueAt(i,2).equals("")||myModel.getValueAt(i,3).equals("")){
                        myModel.removeRow(i);
            ...
            //Clicking Load File From File Menu - "loadFile" command
                ...
                //A Message Dialog To Inform User To Select Invoices Header File First
                ...
                if (choice== JFileChooser.APPROVE_OPTION){
                    ...
                    if (choice== JFileChooser.APPROVE_OPTION){
                        ...
                        //Read from the given 2 paths and combine the header with lines and store the data in the array list
                        invoiceHeaderArrayList = fileOperations.readFile(path1,path2);
                        DefaultTableModel model = gui.getInvoicesTableModel();
                        model.getDataVector().removeAllElements();

                        //Loading loaded data to tables
                        int i=0;
                        for (;i<invoiceHeaderArrayList.size();i++){
                            model.addRow(new Object[]{"","","",""});
